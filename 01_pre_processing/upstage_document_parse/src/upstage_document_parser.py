...
def upstage_document_parse(pdf_file_path):
    ...
    results, split_files = process_pdf_with_split(pdf_file_path)
    
    if split_files:
        # merged_md_path, merged_html_path, merged_items_folder = merge_outputs(results, split_files, pdf_file_path)
        merged_md_path, merged_html_path, merged_items_folder = merge_outputs(results, split_files, pdf_file_path)
        print("최종 병합 결과:")
