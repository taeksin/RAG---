...
def main():
    ...
    # 폴더 내에서 "03_"로 시작하고 "_merged.md"로 끝나는 파일 찾기
    md_file = None
    for fname in os.listdir(folder_path):
        if fname.startswith("03_") and fname.endswith("_merged.md"):
            md_file = os.path.join(folder_path, fname)
            break
    if not md_file:
        raise FileNotFoundError("ERROR: '03_'로 시작하고 '_merged.md'로 끝나는 파일을 찾을 수 없습니다.")
    ...
    # 벡터스토어를 지정한 경로에 저장
    save_path = "vdb/faiss_index/small/03_Nsplit"
